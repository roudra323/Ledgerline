...
| **1 — Ledger core**                    | Ledger tables, immutability trigger, deferred balance trigger, `LedgerService.post()`, `splitFee()`, `convert()` with explicit residual                                                                                                | 10k random postings → trial balance 0; `UPDATE` on an entry throws; projection == recomputed      |
